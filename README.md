...
| **Markdown to PDF** | Markdown から PDF への変換。リアルタイムプレビュー付き |
| **MD Preview** | GFM 対応 Markdown プレビュー。左右2ペイン、テーブル、タスクリスト、コードブロック、目次自動生成、HTMLエクスポート、ダーク/ライトテーマ切替 |
| **Text Diff** | 2つのテキストの差分比較。行単位・文字単位のハイライト表示 |
| **JSON Formatter** | JSON の整形、圧縮、バリデーション、ツリービュー表示、検索 |
| **Char Counter** | 文字数・単語数・行数・バイト数のリアルタイムカウント。UTF-8/Shift_JIS バイト数、文字種別（ひらがな・カタカナ・漢字等）の内訳表示 |
...
| **Unit Converter** | 単位変換。長さ、重さ、温度、データサイズ、時間、面積、体積など多数のカテゴリ対応 |
| **Unix Time** | Unix タイムスタンプと日時の相互変換。秒/ミリ秒、ローカル/UTC 対応 |
| **Regex Tester** | 正規表現のテスト。マッチ結果のハイライト、グループキャプチャ表示、プリセットパターン、置換機能 |
| **Base64** | Base64 エンコード/デコード。テキスト変換、画像の Base64 変換、URL Safe 対応 |
...
| **Shortcuts** | ショートカットキー辞典。VSCode, IntelliJ, Vim, Terminal のキーバインドを検索。macOS/Windows/Linux 対応 |
| **Cheat Sheet** | チートシートビューア。Git, Docker, Kubernetes, tmux, Bash コマンドのクイックリファレンス |
